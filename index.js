...
try {
  ...
  config = {
    ...
    automodLogWebhookUrl: process.env.AUTOMOD_LOG_WEBHOOK,
  };
}
const fs = require("fs");
...
const automodLogWebhookUrl =
  process.env.AUTOMOD_LOG_WEBHOOK || config.automodLogWebhookUrl;

async function sendAutomodLog({ message, reason, warnCount }) {
  if (!automodLogWebhookUrl) return;
  try {
    const webhook = new WebhookClient({ url: automodLogWebhookUrl });
    ...
    await webhook.send({
      embeds: [embed],
      username: "Snoe AutoMod",
      avatarURL: client.user?.displayAvatarURL?.(),
    });
    webhook.destroy();
  } catch (err) {
    console.log("❌ AutoMod log webhook error:", err.message);
  }
}
...
const commands = [
  ...
  new SlashCommandBuilder()
    ...
    .addStringOption((opt) =>
      opt
        .setName("names")
        .setDescription("Channel names separated by commas")
        .setRequired(true),
    )
    .addStringOption((opt) =>
      opt
  ...
  new SlashCommandBuilder()
    ...
    .addIntegerOption((opt) =>
      opt
        .setName("winners")
        .setDescription("Number of new winners to pick (default: 1)")
        .setMinValue(1)
        .setMaxValue(20),
    )
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild)
...
client.once("ready", async () => {
  ...
  for (const [messageId, giveaway] of Object.entries(giveaways)) {
    ...
    if (timeLeft <= 0) {
      ...
      console.log(
        `✅ Restored giveaway timer: ${messageId} (${Math.round(timeLeft / 1000)}s remaining)`,
      );
    }
  }
});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if (interaction.isButton() && interaction.customId.startsWith("marry_")) {
    ...
    if (action === "accept") {
      ...
      if (
        getPartner(interaction.guild.id, odatnerId) ||
        getPartner(interaction.guild.id, targetId)
      ) {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `<@${odatnerId}> and <@${targetId}> are now married!\n\nCongratulations! 🎉`,
        )
        .setTimestamp();
  ...
  if (interaction.commandName === "bulkcreate") {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        ...
        .setDescription(
          `Successfully created **${createdChannels.length}** ${type} channels!\n\n${createdChannels.map((c) => `• ${c}`).join("\n")}`,
        )
        .setThumbnail("attachment://snoe-logo.png");
      ...
      await interaction.editReply({
        content: `❌ Failed to create channels: ${err.message}`,
      });
    }
  }
  ...
  if (interaction.commandName === "mute") {
    ...
    if (!match) {
      return interaction.reply({
        content: "❌ Invalid duration! Use format: 10m, 1h, 1d",
        ephemeral: true,
      });
    }
    ...
    try {
      ...
      await interaction.reply({
        content: `❌ Failed to mute: ${err.message}`,
        ephemeral: true,
      });
    }
  }
  ...
  if (interaction.commandName === "unmute") {
    ...
    try {
      ...
      await interaction.reply({
        content: `❌ Failed to unmute: ${err.message}`,
        ephemeral: true,
      });
    }
  }
  ...
  if (interaction.commandName === "marry") {
    ...
    if (getPartner(interaction.guild.id, target.id)) {
      return interaction.reply({
        content: `❌ ${target.tag} is already married!`,
        ephemeral: true,
      });
    }
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `${interaction.user} has proposed to ${target}!\n\n${target}, do you accept?`,
      );
  ...
  if (interaction.commandName === "divorce") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setTitle("💔 Divorced")
      .setDescription(
        `${interaction.user} has divorced <@${partner.odatnerId}>.`,
      )
      .setTimestamp();
  ...
  if (interaction.commandName === "partner") {
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription(
        `**${target.tag}** is married to <@${partner.odatnerId}>\n\nMarried ${marriedSince}`,
      )
  ...
  if (interaction.commandName === "greroll") {
    ...
    if (!giveaway) {
      return interaction.reply({
        content:
          "❌ Giveaway not found! Make sure you have the correct message ID.",
        ephemeral: true,
      });
    }
  ...
  if (interaction.commandName === "purgeall") {
    try {
      ...
      await interaction.reply({
        content: `Failed to clear channel: ${err.message}`,
        ephemeral: true,
      });
    }
  }
...
client.on("messageCreate", async (message) => {
  ...
  if (message.channel.name.startsWith("ticket-")) {
    ...
    if (ticketData && message.author.id === ticketData.odatnerId) {
      ...
      ticketTimers.set(message.channel.id, {
        odatnerId: ticketData.odatnerId,
        timer: newTimer,
      });
    }
  }
  ...
  // Bypass role check
  if (
    automod.bypassRoleIds?.some((roleId) =>
  ...
  if (!deleted) {
    ...
    const timestamps = messageTracker
      .get(key)
      .filter((t) => now - t < automod.spamInterval);
    timestamps.push(now);
  ...
  if (deleted) {
    try {
      ...
      await sendAutomodLog({ message, reason, warnCount });

      const embed = new EmbedBuilder()
        .setColor(0x2f3136)
        .setDescription(
          `⚠️ <@${message.author.id}>, your message was removed.\n**Reason:** ${reason}\n**Warnings:** ${warnCount}`,
        )
